...
def auth():
    ...
    # ...so 29m as expiry for both

    wx_auth_token_new = 1 ;
    wx_auth_token_txt = 'auth/wx.auth.token.txt' ;
    ...
    if wx_auth_token_new == 1:
        ...
        if cb_post.status_code != 200: print 'ERROR: Non-200 REST API server response (Co-Brand Login), exiting' ; exit(1) ;
        if 'sessionToken' not in cb_post.text: print 'ERROR: Co-Brand Login session token missing, exiting' ; exit(1) ;
        cb_dict = json.loads( cb_post.text ) ;
        cb_auth = cb_dict[ 'cobrandConversationCredentials' ][ 'sessionToken' ] ;

        # user login
        wx_auth_user_txt = 'auth/wx.auth.user.txt' ;
        if os.path.isfile( wx_auth_user_txt ):
            f = open( wx_auth_user_txt , 'r' ) ;
            wx_auth_user_data = f.readlines() ;
            f.close() ;
        else: print 'ERROR: Cannot find/open user auth file' ;
        ...
        if ul_post.status_code != 200: print 'ERROR: Non-200 REST API server response (User Login), exiting' ; exit(1) ;
        if 'sessionToken' not in ul_post.text: print 'ERROR: User Login session token missing, exiting' ; exit(1) ;
